...
export default function Contact() {
    ...
    const handleSubmit = async (e) => {
        ...
        let response = await fetch("https://portfolio-email-z7et.onrender.com/contact", {
          method: "POST",
          headers: {
        ...
        <Container>
            <Row className='align-items-center'>
                <Col size={12} md={6}>
                    <TrackVisibility>
                    {({ isVisible }) =>
                        <img className={isVisible ? "animate__animated animate__zoomIn" : ""} src={contacting} alt="Contact Us"/>
                    }
                    </TrackVisibility>
                </Col>
                <Col size={12} md={6}>
                        ...
                        <br/>
                            {
                                ...
                                <Col className="d-inline-block">
                                    <p className={status.success === false ? "danger" : "text_success"}>{status.message}</p>
                                </Col>
                            }
                        </form>
                        </div>}
